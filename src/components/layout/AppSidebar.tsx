...
export default function AppSidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <motion.aside
        className={`fixed lg:sticky top-0 left-0 h-screen z-50 w-64 flex flex-col border-r border-border bg-sidebar transition-transform lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}
      >
        {/* Logo */}
        ...
        <div className="p-4 border-t border-border space-y-2">
          ...
          >
            ...
            <span className={`ml-auto w-7 h-4 rounded-full transition-colors flex items-center ${anon ? 'bg-primary justify-end' : 'bg-border justify-start'}`}>
              <span className="w-3 h-3 rounded-full bg-primary-foreground mx-0.5 shadow-sm" />
            </span>
          </button>
          <button
