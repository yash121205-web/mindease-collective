...
export default function WellnessSummary({ onClose }: { onClose: () => void }) {
  ...
  const ehsBg = ehs >= 70 ? 'from-secondary/10 to-secondary/5' : ehs >= 40 ? 'from-primary/10 to-primary/5' : 'from-rose-soft/15 to-rose-soft/5';
  ...
  return (
    ...
    >
      ...
      <div className="absolute -bottom-12 -left-12 w-44 h-44 rounded-full bg-secondary/4 blur-3xl pointer-events-none" />
        ...
        >
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <p className="font-display text-base font-semibold text-foreground">Emotional Health</p>
        ...
        <div className="grid grid-cols-3 gap-2.5 mb-5">
          {stats.map((s, i) => (
            <motion.div
              ...
              className="rounded-2xl bg-muted/20 border border-border/15 p-3.5 text-center hover:bg-muted/35 hover:border-border/30 transition-all duration-200 group cursor-default"
            >
              <span className="text-xl block mb-1 group-hover:scale-110 transition-transform duration-200">{s.emoji}</span>
          ...
          >
            ...
            <div className="flex items-end gap-2 h-16">
              {weekMoods.slice(-7).map((m, i) => (
                <motion.div
                  key={i}
                  className="flex-1 rounded-lg relative group"
                  style={{
                    background: m.moodScore >= 75
                      ? 'hsl(var(--secondary))'
                      : m.moodScore >= 50
                        ? 'hsl(var(--primary))'
                        : 'hsl(var(--rose-soft))',
