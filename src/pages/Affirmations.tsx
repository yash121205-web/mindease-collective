      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex gap-2 flex-wrap mb-8">
        {categories.map(cat => (
          <button
            ...
            className={`px-5 py-2.5 rounded-full text-base font-display font-medium transition-all ${category.id === cat.id ? 'bg-primary text-primary-foreground shadow-md' : 'bg-muted/50 text-muted-foreground hover:bg-muted'}`}
          >
            {cat.label}
          </button>
        ))}
