...
import { Phone, Sparkles, BookOpen, Heart, X, ExternalLink, ChevronDown, ChevronUp } from 'lucide-react';
...
const copingGuides = [
  { title: 'Managing Exam Stress', icon: '📝',
  ...
  { title: 'Dealing with Loneliness', icon: '🤝',
  ...
  { title: 'Sleep Hygiene for Students', icon: '😴',
  ...
  { title: 'Understanding Anxiety', icon: '😰',
  ...
  { title: 'How to Talk About Mental Health', icon: '💬',
  ...
  { title: 'Overcoming Procrastination', icon: '⏰',
  ...
  { title: 'Building Emotional Resilience', icon: '💪',
  ...
  { title: 'Healthy Social Media Habits', icon: '📱',
...
const articles = [
  { title: 'Why Your Brain Needs Rest as Much as Your Body', readTime: '4 min', author: 'MindEase Editorial Team',
  ...
  { title: 'The Science of Why Journaling Works', readTime: '3 min', author: 'MindEase Editorial Team',
  ...
  { title: 'Digital Detox: A Student\'s Guide', readTime: '5 min', author: 'MindEase Editorial Team',
  ...
  { title: 'How Sleep Affects Academic Performance', readTime: '4 min', author: 'MindEase Editorial Team',
  ...
  { title: 'Mindfulness in 5 Minutes: A Beginner\'s Guide', readTime: '3 min', author: 'MindEase Editorial Team',
  ...
  { title: 'Understanding the Stress-Performance Curve', readTime: '4 min', author: 'MindEase Editorial Team',
  ...
  const handleAsk = async () => {
    ...
    try {
      const r = await callAI(
        `A student is struggling with: "${query}". Recommend 3 specific, actionable mental wellness strategies with titles and 2-sentence explanations each. Format as JSON [{\"title\":\"...\",\"explanation\":\"...\",\"action\":\"...\"}]. Return ONLY JSON.`,
  ...
  return (
    <div className="p-4 lg:p-8 max-w-4xl mx-auto overflow-y-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-muted-foreground mb-8 font-body">Support when you need it most</p>
                ...
                <div key={h.name} className="flex items-start gap-3 p-3 rounded-xl bg-background/50">
                  ...
                  <div>
                    ...
                    {h.number ? (
                      ...
                      <a href={h.link} target="_blank" rel="noopener" className="text-xs text-primary hover:underline font-body flex items-center gap-1">
        ...
        <section className="mb-8">
          <h2 className="font-display text-xl text-foreground mb-4 font-semibold flex items-center gap-2">
            <BookOpen className="w-5 h-5 text-primary" /> Coping Guides
          </h2>
          <div className="grid gap-3 md:grid-cols-2">
            {copingGuides.map((g, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
                <button
                  onClick={() => setExpandedGuide(expandedGuide === i ? null : i)}
                  className="w-full text-left glass-static rounded-2xl p-4"
                >
                  <div className="flex items-start gap-3">
                    ...
                    {expandedGuide === i ? <ChevronUp className="w-4 h-4 text-muted-foreground shrink-0" /> : <ChevronDown className="w-4 h-4 text-muted-foreground shrink-0" />}
                  </div>
                </button>
                <AnimatePresence>
                  {expandedGuide === i && (
        ...
        <section className="mb-8">
          ...
          <div className="glass-static rounded-2xl p-5">
            <div className="flex gap-2">
              ...
              />
              <button onClick={handleAsk} disabled={!query.trim() || loading} className="btn-primary disabled:opacity-40">
                {loading ? '...' : 'Ask'}
              </button>
            </div>
            {aiResults.length > 0 && (
              <div className="grid gap-3 mt-4 md:grid-cols-3">
                {aiResults.map((r: any, i: number) => (
                  <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                    ...
                    {r.action && <p className="text-xs text-primary font-body mt-2 font-medium">→ {r.action}</p>}
                  </motion.div>
                ))}
              </div>
            )}
        ...
        <section className="mb-8">
          <h2 className="font-display text-xl text-foreground mb-4 font-semibold flex items-center gap-2">
            <Heart className="w-5 h-5 text-primary" /> Wellness Articles
          </h2>
          <div className="grid gap-3 md:grid-cols-2">
            {articles.map((a, i) => (
              <div key={i} className="glass-static rounded-2xl p-5">
                ...
                >
                  {expandedArticle === i ? 'Show less' : 'Read more →'}
                </button>
                <AnimatePresence>
                  {expandedArticle === i && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
