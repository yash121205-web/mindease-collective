...
function EaseAvatar({ emotion }: { emotion?: string }) {
  const tilt = emotion === 'Sadness' || emotion === 'Loneliness' ? 'rotate-[-8deg]' : '';
  const glow = emotion === 'Happiness' || emotion === 'Gratitude';
  return (
    <div className={`w-9 h-9 rounded-full bg-primary/15 flex items-center justify-center animate-float ${tilt} transition-transform`}>
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      const errMsg: ChatMessage = {
        id: genId(),
        role: 'assistant',
        content: "Ease is taking a moment to think... Please try again. 💙",
        timestamp: Date.now(),
      };
    ...
    <div className="flex flex-col h-[calc(100vh-3.5rem)] lg:h-screen">
        ...
        {showCrisis && (
          ...
          >
            ...
            <p className="text-xs text-foreground">
              If you're in crisis, please reach out: <strong>iCall: 9152987821</strong> · <strong>Vandrevala Foundation: 1860-2662-345</strong> · <strong>AASRA: 9820466627</strong>
            </p>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 lg:p-6 space-y-4">
        {messages.length === 0 && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center pt-16 lg:pt-24">
            ...
            <div className="mx-auto mt-4 mb-2">
              <h2 className="font-display text-2xl text-foreground">Hi, I'm Ease 💙</h2>
              <p className="text-muted-foreground text-sm mt-1 max-w-sm mx-auto">
                Your empathetic AI companion. I'm here to listen, support, and help you navigate your feelings.
              </p>
            </div>
            <div className="flex flex-wrap justify-center gap-2 mt-6">
              {SUGGESTED.map(s => (
                <button
                  ...
                  className="px-4 py-2 rounded-2xl glass text-sm text-foreground hover:shadow-md transition-all hover:scale-[1.02]"
                >
                  {s}
                </button>
              ))}
        ...
        {messages.map((msg) => (
          ...
          >
            {msg.role === 'assistant' && <EaseAvatar emotion={msg.emotion} />}
            <div className={`max-w-[75%] lg:max-w-[60%]`}>
              ...
              <div className="flex items-center gap-2 mt-1 px-1">
                ...
                {msg.role === 'assistant' && (
                  <button className="text-[10px] text-muted-foreground hover:text-primary flex items-center gap-0.5 ml-1">
      ...
      <div className="border-t border-border p-4 bg-background/80 backdrop-blur-lg">
        <div className="max-w-3xl mx-auto flex gap-2">
          <textarea
            ...
            rows={1}
            className="flex-1 resize-none bg-muted rounded-2xl px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 transition-all"
          />
          <button
            onClick={() => sendMessage(input)}
            disabled={!input.trim() || loading}
            className="w-11 h-11 rounded-2xl bg-primary text-primary-foreground flex items-center justify-center disabled:opacity-40 hover:opacity-90 transition-opacity shrink-0"
          >
